...
IFS=' ' read -r -a hide_status <<< $(get_tmux_option "@neom-fossil-disable-status" "false")
IFS=' ' read -r -a current_symbol <<< $(get_tmux_option "@neom-fossil-show-current-symbol" "✓")
IFS=' ' read -r -a diff_symbol <<< $(get_tmux_option "@neom-fossil-show-diff-symbol" "!")
IFS=' ' read -r -a no_repo_message <<< $(get_tmux_option "@neom-fossil-no-repo-message" "")
IFS=' ' read -r -a no_untracked_files <<< $(get_tmux_option "@neom-fossil-no-untracked-files" "false")
IFS=' ' read -r -a show_remote_status <<< $(get_tmux_option "@neom-fossil-show-remote-status" "false")
...
# getting the #{pane_current_path} from neom.sh is no longer possible
getPaneDir()
{
 nextone="false"
